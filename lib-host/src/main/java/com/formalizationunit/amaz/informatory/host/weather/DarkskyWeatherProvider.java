...
import com.formalizationunit.amaz.informatory.host.weather.net.Net;
...
public class DarkskyWeatherProvider implements WeatherProvider {
    ...
    private final ApiKeyProvider mApiKeyProvider;
    private final GeocodingProvider mGeocodingProvider;
    private final String mSpeedUnit;

    public DarkskyWeatherProvider(Context context, ApiKeyProvider apiKeyProvider) {
        mApiKeyProvider = apiKeyProvider;
        mGeocodingProvider = new GeocodingProvider(apiKeyProvider);

        mSpeedUnit = context.getResources().getString(R.string.speed_unit);
    }

    public void request(double lat, double lon, Callback callback) {
        String url = HOST + mApiKeyProvider.weatherApiKey() + "/" + lat + "," + lon + PARAMS;
    ...
    private void hourParse(JSONObject in, JSONObject out) throws JSONException {
        ...
        int weatherCode = translateWeatherIcon(in.optString("icon"));
        out.put("weatherCode", weatherCode);
        out.put("weatherCodeFrom", weatherCode);
